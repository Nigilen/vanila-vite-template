	...
	for (const image of images) {
		...
		if (imageExtName) {
			...
			for (const encodedImage of Object.values(targetImage.encodedWith)) {
				writeFile(
					`${imageWithOutExtName}.${encodedImage.extension}`,
					encodedImage.binary,
					() => {
						console.info(
							`Image path: ${imageWithOutExtName}.${encodedImage.extension}`
						)
					}
				)
			}
			unlink(image.path, (error) => {
				...
				console.log(`Файл ${image.path} удален`)
			})
		}
	}
}
...
// npm run squoosh
// следите за версией node.js, она должна быть не выше 16 версии
